...
/**
 * ESP-NOW 공통 페이로드 구조체 (8 Bytes)
 * 모든 노드 공유 — 수정 시 전체 노드 재컴파일 필요
 */
typedef struct {
    uint8_t msg_type;     /* 0: keep-alive  1: 센서 트리거  2: 제어 명령  3: 센서 데이터 */
    ...
    uint8_t temperature;  /* 센서 온도 (°C, 0~50)                         */
    uint8_t humidity;     /* 센서 습도 (%, 0~99)                          */
