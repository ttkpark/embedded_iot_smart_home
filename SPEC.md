...
└─────────────────────────────────────────────────────────────────┘
   ...
   ┌──────┼──────┐                          ┌───────┼───────┐
   │      │      │                          │               │
SSR    IR LED  SG90-HV                 버튼1(정상)  버튼2(응급)
릴레이  에어컨  Continuous Servo
 환풍기        창문 개폐
```
...
| `GPIO 27` | SG90-HV Continuous Servo (Signal) | 창문 개폐 구동 | 50Hz PWM. 1500us=정지, 1700us/1300us=방향 제어 |

> **[Hardware Tip]** GPIO 25, 27에는 **10kΩ 풀다운 저항 부착을 권장**합니다. 전원 인가 순간 핀이 플로팅 상태가 되어 릴레이나 서보가 순간 오작동(Glitch)하는 현상을 줄일 수 있습니다.
...
#### SG90-HV Continuous 연결 (참고)

```
Signal(GPIO 27) → Servo Signal(주황/흰색)
VCC(5~6V)       → Servo VCC(빨간색)
GND             → Servo GND(갈색/검정색), ESP32 GND와 공통 접지
...
| Node B | 27 | SG90-HV Continuous PWM | 창문 제어용 50Hz PWM 출력 |
...
#define SERVO_PULSE_OPEN_US 1700
#define SERVO_PULSE_CLOSE_US 1300
...
void controlWindow(uint8_t action) {
    switch(action) {
        case 0:  // 정지(중립)
            servoWriteUs(SERVO_PULSE_STOP_US);
            break;
        case 1:  // 열림 방향
            servoWriteUs(SERVO_PULSE_OPEN_US);
            delay(MOTOR_RUN_MS);  // 이동 완료 후 자동 정지
            servoWriteUs(SERVO_PULSE_STOP_US);
            break;
        case 2:  // 닫힘 방향
            servoWriteUs(SERVO_PULSE_CLOSE_US);
            delay(MOTOR_RUN_MS);
            servoWriteUs(SERVO_PULSE_STOP_US);
            break;
    }
...
> `MOTOR_RUN_MS`와 `SERVO_PULSE_*_US`는 실제 창문 개폐 시간/방향에 맞게 캘리브레이션 필요.
...
Node A ESP-NOW 수신 콜백(OnDataRecv) 실행
    ...
    └─→ ESP-NOW 전송: {msg_type=2, fan=1, ac=18, window=1} → Node B
        ...
        Node B OnDataRecv 실행
            ...
            └─→ GPIO 27 PWM(1700us) → SG90-HV 회전(열림 방향) → 창문 열림
```
